...
@group(0) @binding(0) var<uniform> overlay: StartupProgressOverlay;
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
	let uv = vec2(in.ndc.x * overlay.aspect, in.ndc.y);
	let r = length(uv);
	let angle = atan2(uv.y, uv.x);
	let tau = 6.28318530718;
	let t = overlay.time;
	let pulse = 0.5 + 0.5 * sin(t * 2.2);
	let determinate = overlay.progress >= 0.0;
	let progress = clamp(overlay.progress, 0.0, 1.0);
	let phase_shift = overlay.phase * 0.43;
	...
	let failed = overlay.phase >= 8.0;
	let healthy_accent = mix(mint, mix(gold, pink, fract(overlay.phase * 0.37)), 0.26 + 0.16 * sin(t + overlay.phase));
