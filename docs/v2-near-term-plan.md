...
- Mesh Cutter / Shape Changer / VertexFilter component payload は metadata として保持しつつ MeshCutter / ShapeChanger を resolver-capable に分類する。CLI diagnose は target、combine mode、blendshape / mask / bone / axis filter summary を観測できる。Runtime resolver は blendshape-based delete filters を、MA と同じく morph delta threshold で頂点選択し、axis delete filters を `dot(axis, vertex-center) > 0`、bone delete filters を skin joint weight の正規化比率で頂点選択し、該当頂点を含む三角形を削除する。mask filter は Exporter が concrete texture asset / sampler reference を保存してから実装する。SkinnedMeshRenderer BakeMesh 相当の axis evaluation、動的 reactive gating はまだ未実装。
- Mask filter texture は既存 root `textureAssets` に統合し、MeshCutter / VertexFilter component payload は `maskTextureAssetId`、material index、delete mode だけを持つ。MeshCutter 専用 asset pool は作らない。Exporter は exportable な Mask texture を追加 texture asset として保存し、payload から参照できる。
