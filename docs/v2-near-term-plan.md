...
- Modular Avatar Material Setter の direct renderer slot payload と Material Swap の scene-aware From / To slot expansion は `MaterialSlot` action へ import される。Generic wardrobe material color / scalar / slot operations も hot switch で適用される。残りは Menu Item / parameter trigger 統合、QuickSwapMode / asset group 対応。
