...
- wardrobe asset group 単位の lazy GPU upload / unload。現在は選択 set の `assetGroups`、最小 resolver cache key、source asset group ownership count、active group から算出した scoped resident mesh/material/image/dynamics count を runtime state / renderer status / CLI diagnose / wardrobe apply report で観測する段階。実 GPU resource の scoped upload / unload はまだ `all-resident` fallback。
