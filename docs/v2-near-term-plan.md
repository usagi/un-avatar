...
- wardrobe asset group 単位の lazy GPU upload / unload。現在は選択 set の `assetGroups`、最小 resolver cache key、source asset group ownership count、active group から算出した scoped resident mesh/material/image/dynamics count と missing active group、renderer draw residency count、image texture slot residency count、inactive image slot を参照する draw count、active draw が参照する inactive image/material slot count と bounded slot index preview、material slot residency count、pending scoped texture/material upload work count、直近 hot switch refresh の image/material load/unload transition count、image texture scoped load/unload count、material slot scoped upload count を runtime state / renderer status / CLI diagnose / wardrobe apply report で観測する段階。renderer draw/material/image residency は core scoped asset selection と同じく unowned asset を resident のまま扱い、source ownership がある asset だけ active group で絞る。renderer 内部では active group の source asset work list、active draw が要求する texture/material gap の full upload work list、hot switch refresh 時の image/material load/unload transition を分離済み。hot switch 時は material slot / asset residency 更新後、active draw が要求する inactive 2D image texture slot を CPU upload payload から GPU texture へ scoped upload し、inactive image は fallback view へ差し替えて GPU texture/view を drop し、active draw が要求する inactive material slot を resident に昇格してから material bind group を再同期する。Unity Exporter は renderer / PhysBone source path の top-level name から、既存 capture と同じ `outfit:<top>` group を推定し、wardrobe set が宣言している group だけ `wardrobe.assetGroupOwnership` へ出す。cubemap resource の scoped upload / unload はまだ `all-resident` fallback。
