...
- Windows Jump List tasks should expose profile launch tasks and `Open Supervisor` under the same stable launcher AppUserModelID. Profile tasks may be generated from Supervisor's visible profile list, use deterministic titles, and pass the selected manifest/profile argument to the launcher. They should not require Supervisor to stay resident.
