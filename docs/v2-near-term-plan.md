...
- Mesh Cutter / Shape Changer / VertexFilter component payload は metadata として分類し、core に共通 vertex filter group representation を追加した。CLI diagnose は target、combine mode、blendshape / mask / bone / axis filter summary を観測できる。現時点では resolver-side vertex selection と mesh mutation はまだ未実装。
