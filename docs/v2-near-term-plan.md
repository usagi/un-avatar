...
- 残り: VRC PhysBone Collider の detailed behavior、stretch limit behavior、grabbing / posing action hooks、Contacts diagnostics-only probe / opt-in emission、VRC Constraints solver integration、animation state と連動した runtime enable state。
...
- Contacts は v2 初期範囲の metadata + diagnostics と parameter declaration を core runtime view / renderer runtime status / CLI diagnose まで接続済み。Diagnostics-only contact probe は debug 機能として追加してよいが、parameter emission は profile flag または `.unavatar` capability による opt-in とし、実サンプルで誤爆と座標系を検証するまで既定有効にしない。
