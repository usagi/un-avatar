...
4. Runtime CPU
   ...
   - `image_source_metadata.detail` profile は profiled import 時だけ有効にし、通常起動 path へ per-image atomic timing を入れない。`mizuki-split / field_drape` では `image_source_metadata_ms=152ms`、thread 合算で `dimensions_ms=1.8`、`hash_ms=472.7`、`hash_mb=561.6`。metadata 停止の主因は encoded image dimensions ではなく texture cache key 用 source hash。次に触る場合は cache stale を起こさない hash / file identity 設計が必要。
