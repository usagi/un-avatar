...
- 現在対応済み: VRC PhysBone `rootTransform` / `ignoreTransforms` / `multiChildType=Ignore` / `endpointPosition` / `radius` / `pull` / `spring` / `stiffness` / `gravity` / `allowCollision=false` の最小抽出と lower、source collider metadata 保存、branch root の複数 group 化、CLI diagnostics。
- 残り: VRC PhysBone insideBounds collider / limit / detailed collision behavior / grabbing / posing の再現、wardrobe / action state と連動した runtime enable state。
...
- 現在は exporter/importer が PhysBone source を runtime dynamics group / collider data へ lower し、endpointPosition は leaf root の synthetic child として正規化し、normalized collider data を既存 SpringBone solver collider へ接続している。`allowCollision=false` は source collider を solver へ渡さない。CLI diagnose と renderer runtime status が raw/lowered 件数を観測できる。残りは insideBounds collider、limits、detailed collision behavior、grabbing/posing、wardrobe/action runtime enable。
