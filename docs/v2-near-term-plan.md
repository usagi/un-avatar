...
- Contacts は v2 初期範囲の metadata + diagnostics と parameter declaration を core runtime view / CLI diagnose まで接続済み。Diagnostics-only contact probe は debug 機能として追加してよいが、parameter emission は profile flag または `.unavatar` capability による opt-in とし、実サンプルで誤爆と座標系を検証するまで既定有効にしない。
