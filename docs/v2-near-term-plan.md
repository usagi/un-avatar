...
- 残り: stretch / endpoint edge case、grabbing / posing action hooks、VRC Constraints solver integration、animation state と連動した runtime enable state。per-chain radius curve は base radius 倍率を chain tail ごとの `hit_radius_samples` として solver collider constraint へ近似反映する初期実装まで完了。これを v2 初回リリースの主要未完了領域として扱う。
