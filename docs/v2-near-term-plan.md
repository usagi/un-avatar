...
- Contacts は v2 初期範囲の metadata + diagnostics と parameter declaration を core runtime view / renderer runtime status / CLI diagnose まで接続済み。Diagnostics-only contact probe は CLI diagnose の static scene pose probe として追加済み。parameter emission は profile flag または `.unavatar` capability による opt-in とし、実サンプルで誤爆と座標系を検証するまで既定有効にしない。
