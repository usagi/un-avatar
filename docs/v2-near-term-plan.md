...
- wardrobe asset group 単位の lazy GPU upload / unload。現在は選択 set の `assetGroups`、最小 resolver cache key、source asset group ownership count、active group から算出した scoped resident mesh/material/image/dynamics count と missing active group、renderer draw residency count / mesh buffer byte residency、image texture slot residency count、inactive image slot を参照する draw count、active draw が参照する inactive image/material slot count と bounded slot index preview、material slot residency count、pending scoped texture/material upload work count、直近 hot switch refresh の mesh buffer load/unload count、image/material load/unload transition count、image/cubemap texture scoped load/unload count、material slot scoped upload count を runtime state / renderer status / CLI diagnose / wardrobe apply report で観測する段階。renderer draw/material/image residency は core scoped asset selection と同じく unowned asset を resident のまま扱い、source ownership がある asset だけ active group で絞る。renderer 内部では active group の source asset work list、active draw が要求する texture/material gap の full upload work list、hot switch refresh 時の mesh/image/material load/unload transition を分離済み。hot switch 時は material slot / asset residency 更新後、active draw の mesh buffer を CPU upload payload から scoped upload し、inactive draw の mesh buffer を drop する。active draw が要求する inactive image texture / cubemap slot も CPU upload payload から GPU texture へ scoped upload し、inactive image は fallback view へ差し替えて GPU texture/view を drop し、active draw が要求する inactive material slot を resident に昇格してから material bind group を再同期する。Unity Exporter は renderer / PhysBone source path の top-level name から、既存 capture と同じ `outfit:<top>` group を優先し、宣言済み non-outfit group の suffix が top-level name と一意に一致する場合も `wardrobe.assetGroupOwnership` へ出す。残りは ambiguous group name の explicit mapping と broader eviction policy。
