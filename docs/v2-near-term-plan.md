...
- 残り: VRC PhysBone Collider の detailed behavior、stretch limit behavior、grabbing / posing action hooks、Contacts evaluation、VRC Constraints solver integration、animation state と連動した runtime enable state。
...
- Contacts は v2 初期では metadata + diagnostics と parameter declaration までを目標にする。Diagnostics-only contact probe は debug 機能として追加してよいが、parameter emission は profile flag または `.unavatar` capability による opt-in とし、実サンプルで誤爆と座標系を検証するまで既定有効にしない。
