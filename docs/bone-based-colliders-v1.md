...
`dynamics_*` は renderer が正規化後に保持している runtime dynamics group / collider / contact / constraint ref の件数であり、raw `.unavatar` `dynamics` entry 数ではない。`dynamics_enabled_group_count` は wardrobe / action 適用後に solver 対象として残っている group 数、`dynamics_source_enabled_group_count` は authored default、`dynamics_enabled_override_count` は runtime state override 数。limit / grabbing / posing は metadata count であり、stretch / interaction 挙動の solver 反映を意味しない。raw entry と runtime group の食い違いは `un-avatar-cli diagnose` の warning で検出する。
