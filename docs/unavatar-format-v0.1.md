...
`baseSet` は reset / fallback / startup の基準になる安全な初期表示状態であり、裸の素体である必要はない。v0.1 では Base を空文字列 set id `""` として扱い、profile の `wardrobe_set` 未指定 / 空文字もこの Base を選択する。むしろ配信用途では、意図しない wardrobe reset や操作ミスで露出事故が起きないよう、デフォルト衣装込みの状態を `baseSet` として保存する運用を推奨する。素体は source graph の一部であって、ユーザーに表示される baseline wardrobe state とは別概念である。
...
- `dynamicsEnable` は node target ではなく `{ "dynamicsId": "..." }` を使う。`dynamicsId` は `dynamics[].id` を参照する。
  Runtime はこれを正規化済み dynamics group の `enabled` に反映し、disabled group は solver 対象外にする。
...
`nodeConstraints[]` は scene transform evaluation へ直接 lower される source-neutral constraint であり、Unity の authored transform relation を runtime node graph に反映する。`parent`、`unity_aim`、`unity_rotation`、`unity_position`を扱い、いずれも target node、順序付き source node 群、全体 weight、source ごとの weightを保持する。`parent` は source ごとの translation / rotation offset、translate / rotate axis flags、at-rest 値も保持する。`unity_aim` は glTF座標系の aim / up / world-up vector、world-up mode / object、rotation axis flags、at-rest / offset quaternionを保持する。`unity_rotation`はrotation axis flagsとat-rest / offset quaternion、`unity_position`はtranslation axis flagsとat-rest / offsetを保持する。sourceが複数ある場合も単一targetに対するweighted source listとして保存し、Modular Avatar Merge ArmatureやReplace Object適用後もtarget/source/world-up node参照をremapして維持する。モデル名、衣装名、骨名による推測は行わない。Unity `ScaleConstraint`と`LookAtConstraint`もsource payloadには保存するが、現在はruntime未対応としてimport warningとlost featureへ明示する。
