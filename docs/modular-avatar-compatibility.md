...
- `[~]` MA Mesh Cutter
  ...
  - remaining: dynamic reactive mesh gating beyond enabled static payloads is deferred to full Animator graph evaluation. Unity `TextureWrapMode.MirrorOnce` is preserved in `UN_avatar.textureAssets[].sampler` and honored by CPU Mask filter sampling; GPU material sampling falls back to clamp because WGPU has no MirrorOnce address mode.
- `[~]` MA Shape Changer
  ...
  - remaining: dynamic reactive mesh gating beyond enabled static payloads is deferred to full Animator graph style evaluation.
...
- `[~]` vertex filters: by blendshape, mask, bone, axis
  ...
  - remaining: dynamic reactive mesh gating is deferred to full Animator graph evaluation. Unity `TextureWrapMode.MirrorOnce` is resolver-compatible for Mask filters, with GPU material sampling using clamp fallback.
...
- `[~]` MA Menu Item / Menu Group / Menu Installer
  - done: exporter extracts menu item hints. Runtime / diagnose classify Menu Item, Menu Group, Menu Installer, Menu Install Target, and synthetic VRC Expressions Menu controls as metadata components rather than unsupported behavior, and CLI diagnose reports saved label, control type, parameter/value, component index, hierarchy path, sibling index, target path, menu source, source target, menuToAppend, install target menu, and installer reference from the captured payload.
  - done: CLI diagnose derives deterministic menu graph candidates from component index, hierarchy path, sibling index, menu source target, and installer references, then promotes them into graph nodes with parent component edges and child component lists for hierarchy-based UI labels. Diagnose also reports installer / install-target edges and marks an installer's target-menu edge as ignored when any Menu Install Target references that installer, matching Modular Avatar's filtering rule.
  - done: MenuItem parameter/value metadata is matched to effect-backed runtime actions through action conditions first, then parameter triggers, so UI candidates can report the action id, effect kinds, and WardrobeSet ids they would drive. Diagnose groups MenuItem -> runtime action -> WardrobeSet matches into `menu_wardrobe_candidates` with menu path labels for wardrobe UI planning.
  - done: Supervisor wardrobe menu candidates activate renderer runtime actions by `action_id` directly. Renderer UNAnimator actions render all filtered runtime actions inside a scrollable panel instead of hiding them behind an app-side fixed cap. Renderer tray exposes wardrobe and UNAnimator candidates from normalized renderer status, and profile input bindings can assign keyboard or MIDI Note accelerators to wardrobe sets and UNAnimator actions.
  - done: CLI diagnose and renderer status mark `menu_path_truncated` if a malformed menu graph cycle or invalid parent index forces path walking to stop. Unity Exporter preserves referenced `VRCExpressionsMenu` asset path / guid and bounded control metadata for `menuToAppend`, install target menus, submenu references, and `VRCAvatarDescriptor.expressionsMenu` root controls; CLI diagnose and renderer status assign stable synthetic `menu_key` values and expand external or descriptor-root asset controls into menu action / wardrobe candidates when their parameter/value matches an effect-backed runtime action.
  - remaining: richer Supervisor hierarchy/search UI and full external menu UI parity beyond normalized runtime actions, tray access, and bounded diagnostics.
- `[~]` MA Parameters
  - done: `ModularAvatarParameters` is classified as metadata and CLI diagnose reports captured `ParameterConfig` entries with name/prefix, remap target, internal/prefix flags, sync type, local-only state, default value, saved state, explicit-default flag, and effective animator-default override flag. The effective flag follows Modular Avatar's `OverrideAnimatorDefaults` property: an explicit override is honored, and an unsynced parameter with a default value is also treated as an animator-default override. Runtime parameter definitions also treat `syncType=NotSynced` as effective local-only, matching Modular Avatar's `CollectParameters` local-only resolution. Core runtime model now exposes source-neutral runtime parameter definitions from action triggers / conditions, contact receivers, runtime state, and concrete `ModularAvatarParameters` metadata, plus contact-vs-action, Modular Avatar parameter type, and Modular Avatar default-value conflict diagnostics. `isPrefix` entries are kept as diagnose metadata but are not treated as concrete runtime parameter names, matching Modular Avatar's separate parameter-remapping path. Renderer attach applies missing runtime parameter initial values from Modular Avatar defaults without overwriting existing runtime state. CLI diagnose, renderer runtime status, and Supervisor diagnostics surface those definitions / conflicts.
  - remaining: actual AnimatorController default-value rewriting from those overrides and conflict policy for future non-MA parameter sources.
...
- `[x]` resolver cache key
  - done: runtime resolver cache key exposes selected wardrobe set, active asset groups, Modular Avatar component payload hash, material source hash, mesh render identity hash, and resolver version through core, CLI diagnose, and renderer status. Mesh render identity includes vertex attribute layout, vertex color presence, indices, positions, weights, joints, morph target deltas, and default morph weight payload hashes, so resolver mesh mutations such as Remove Vertex Color, Mesh Cutter, Shape Changer, and skinned axis filter output can invalidate downstream mesh caches even when buffer lengths stay unchanged.
