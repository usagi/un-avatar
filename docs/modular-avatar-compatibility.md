...
- `[~]` MA Mesh Cutter
  ...
  - remaining: Unity `TextureWrapMode.MirrorOnce` needs exporter schema parity instead of being folded into glTF mirrored-repeat, plus SkinnedMeshRenderer BakeMesh-equivalent axis evaluation, generated mesh cache key detail, and dynamic reactive gating beyond enabled static payloads.
- `[~]` MA Shape Changer
...
- `[~]` vertex filters: by blendshape, mask, bone, axis
  ...
  - remaining: Unity `TextureWrapMode.MirrorOnce` parity, SkinnedMeshRenderer BakeMesh-equivalent axis evaluation, and dynamic reactive gating.
