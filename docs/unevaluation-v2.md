...
4. Phase D: Opt-in parameter emission
   ...
   - 既存 runtime parameter state は保持し、contact receiver が宣言する parameter だけを更新する。
   - Renderer frame loop では parameter 値が変化した時だけ既存 runtime action evaluator へ流す。
   - owner key は `contact:<source_id>`。

v2 初期リリース目標は Phase A + Phase B まで。
Phase C は debug-only diagnostics として追加済み。
Phase D は opt-in 時のみ current runtime scene pose probe から runtime parameter state へ 1/0 を書き、変更時に runtime action evaluator へ流す初期実装まで追加済み。既定 OFF は維持する。CLI diagnose / renderer runtime status は probe が would_emit だが contact parameter emission が無効な場合、parameter が書かれない状態を receiver/source path、sender source、parameter sample 付き warning として出す。
