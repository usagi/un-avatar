...
- Metadata reads must not pull the whole GLB BIN payload. The current contract is a bounded JSON chunk plus referenced preview bufferView ranges only; wardrobe set discovery also uses the same JSON-chunk path.
