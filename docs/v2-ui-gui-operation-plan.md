...
- Metadata reads must not pull the whole GLB BIN payload. The current contract is a bounded JSON chunk plus referenced preview bufferView ranges only; data URI and external preview image reads are also size-capped before decode / full read. Wardrobe set discovery uses the same JSON-chunk path.
