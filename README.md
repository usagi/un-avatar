...
U.N. Avatar は VRM と VRC 向け Unity アバターを、配信向けに軽快に表示する「仮想アバターレンダラー」です。姿勢や表情の入力を U.N. Motion からの UNMF/Z や VMC 対応で受け、透過ウィンドウ、スクリーンショット保存、Spout2 出力などを行えます。

v2 では従来の VRM / MToon / SpringBone 中心の対応に加えて、VRC モデル由来の `.unavatar`、lilToon-compatible material、PhysBone 由来の dynamics、wardrobe set を扱えます。VRC モデルは Unity 上の U.N. Avatar Exporter で `.unavatar` に変換してから Supervisor / Renderer で使います。

## はじめに

- VRM モデルを使う場合: Supervisor Console で profile を作り、`.vrm` を選んで Renderer を起動します。
- VRC / Unity アバターを使う場合: Unity project に U.N. Avatar Exporter を入れ、対象 avatar を `.unavatar` に export してから profile に設定します。
- 衣装や小物を切り替えたい場合: Exporter で wardrobe set を含めた `.unavatar` を作り、Renderer tray または Supervisor から切り替えます。
- 配信に載せる場合: Window preview、transparent window、Spout2 output のいずれかを profile ごとに選びます。
...
- VRM だけでなく、VRC 向け Unity avatar をローカル配信環境で使いたい民
- UNMF/Z、VRM、`.unavatar`、UNToon / lilToon-compatible 表示の検証、ツール連携、研究用途で renderer を直接扱いたい研究者 / 開発者
...
- VRM SpringBone と VRC PhysBone 由来 dynamics を U.N. Avatar の runtime dynamics として扱う。
- `.unavatar` wardrobe set、VRC Expression Menu / Animator 由来の runtime action、lilToon-compatible UNToon material を扱う。
...
- VRC モデルも使いやすい: Unity Exporter が VRC avatar、lilToon material、PhysBone、Expression Menu / Animator、wardrobe set を `.unavatar` にまとめ、Renderer はそれを Unity なしで読み込みます。
