<script lang="ts">
  ...
  import {
    fallbackVrmMetadata,
    looksLikeVrmPath,
    type VrmMetadataDialogState,
    type VrmMetadataInfo,
  } from "./lib/vrmMetadata";
  import {
    loadAppSettings,
    saveAppSettings,
    type AppSettings,
  ...
  import {
    cameraOrbitPresetAngles,
    type CameraOrbitPreset,
  ...
  import {
    canApplyWithoutRestart,
    isLaunchTimeRendererField,
    isRuntimeWindowField,
    profileFieldLabel,
  } from "./lib/profileFieldRules";
  import {
  ...
  import {
    defaultSpringBoneCategoryOverrides,
  } from "./lib/springBonePresets";
  import {
  ...
  let activeTab = $state<"renderers" | "settings" | "logs" | "app">(
    "renderers",
  );
  ...
  let appSettings = $state<AppSettings>(
    loadAppSettings(appSettingsStorageKey, legacyThemeModeStorageKey),
  );
  ...
  const visibleRenderers = $derived(
    renderers.filter(
      (renderer) => renderer.state !== "Exited" && renderer.state !== "Crashed",
    ),
  );
  const rendererTableRenderers = $derived(
    showStoppedRenderers ? renderers : visibleRenderers,
  );
  ...
  const selectedRenderer = $derived(
    selectedRendererById(selectedRendererId) ??
      rendererTableRenderers[0] ??
      null,
  );
  const selectedRuntimeStatus = $derived(
    selectedRenderer ? (runtimeStatuses[selectedRenderer.id] ?? null) : null,
  );
  const selectedSetting = $derived(
    avatarSettingBySelectedId(selectedSettingId) ??
      avatarSettings[0] ??
      null,
  );
  const launchProfileId = $derived(
    launchTargetId?.startsWith("group:") ? null : launchTargetId,
  );
  const launchTargetSetting = $derived(
    avatarSettingBySelectedId(launchProfileId) ??
      selectedSetting,
  );
  const profileGroups = $derived(
    Array.from(avatarSettingsByGroup.keys()).sort((a, b) => a.localeCompare(b)),
  );
  ...
  const launchGroupSettings = $derived(
    launchGroupName
      ? (avatarSettingsByGroup.get(launchGroupName) ?? [])
      : [],
  );
  ...
  const resolvedTheme = $derived(
    appSettings.theme_mode === "system" ? osTheme : appSettings.theme_mode,
  );
  const diagnosticsSearchQuery = $derived(
    diagnosticsSearch.trim().toLowerCase(),
  );
  const filteredDiagnosticsExports = $derived(
    diagnosticsSearchQuery
      ? diagnosticsExports.filter((entry) =>
          diagnosticsEntrySearchText(entry).includes(diagnosticsSearchQuery),
        )
      : diagnosticsExports,
  );
  ...
  const diagnosticsPreviewMatchCount = $derived(
    diagnosticsPreviewText && diagnosticsSearchQuery
      ? countTextMatches(diagnosticsPreviewText, diagnosticsSearchQuery)
      : null,
  );
  const diagnosticsPreviewBundle = $derived(
    diagnosticsPreviewText ? diagnosticsBundleFromText(diagnosticsPreviewText) : null,
  );
  ...
  const diagnosticsPreviewFindings = $derived(
    diagnosticsPreviewBundle ? diagnosticsBundleFindings(diagnosticsPreviewBundle) : [],
  );
  ...
  const diagnosticsCompareBundles = $derived(
    (() => {
      ...
      for (const entry of diagnosticsCompareEntries) {
        const bundle = diagnosticsBundleFromText(
          diagnosticsCompareTexts[entry.path] ?? "",
        );
  ...
  const diagnosticsCompareSummary = $derived(
    diagnosticsCompareEntries.length === 2
      ? diagnosticsComparisonSummary(
          diagnosticsCompareEntries[0],
          diagnosticsCompareEntries[1],
        )
      : null,
  );
  const diagnosticsCompareDetails = $derived(
    diagnosticsCompareEntries.length === 2 && diagnosticsCompareBundles.length === 2
      ? diagnosticsComparisonDetails(
          diagnosticsCompareBundles[0],
          diagnosticsCompareBundles[1],
        )
      : [],
  );
  ...
  function updateActiveProfileSectionFromScroll(event: Event): void {
    ...
    const sections = Array.from(
      scroller.querySelectorAll<HTMLElement>("[data-profile-section]"),
    );
  ...
  function replaceAvatarSetting(setting: AvatarSetting): void {
    const next = avatarSettings.filter(
      (item) =>
        item.id !== setting.id &&
        item.manifest_path !== setting.manifest_path,
    );
    next.push(setting);
  ...
  async function persistLastSelectedSettingId(
    id: string | null,
  ): Promise<void> {
    if (!hasTauriRuntime()) return;
    if (!backendAppSettingsReady) return;
  ...
  async function loadBackendAppSettings(): Promise<void> {
    ...
    try {
      ...
      const effective =
        appSettings.locale ||
        (await invoke<string>("i18n_resolve_default_locale").catch(() => "ja-JP"));
  ...
  function rendererRuntimeStatus(
    renderer: RendererInstance,
  ): RendererRuntimeStatus | null {
  ...
  async function toggleDiagnosticsCompare(
    entry: DiagnosticsExportEntry,
  ): Promise<void> {
    if (diagnosticsComparePaths.includes(entry.path)) {
      diagnosticsComparePaths = diagnosticsComparePaths.filter(
    ...
    diagnosticsComparePaths = [
      ...diagnosticsComparePaths.slice(-1),
      entry.path,
    ];
  }
  ...
  function isLiveRenderer(renderer: RendererInstance): boolean {
    return (
      renderer.pid != null &&
      renderer.state !== "Exited" &&
      renderer.state !== "Crashed"
    );
  }
  ...
  function rendererForSetting(
    setting: AvatarSetting | null,
  ...
  function syncSelectionFromRendererActivation(
    statuses: RendererRuntimeStatus[],
    ...
    for (const status of statuses) {
      ...
      if (
        ...
        (!activated ||
          status.window_activation_seq > activated.window_activation_seq)
      ) {
        activated = status;
      }
    }
  ...
  function restartableRendererForField(field: string): RendererInstance | null {
    ...
    return isLaunchTimeRendererField(field)
      ? rendererForSetting(selectedSetting)
      : null;
  }

  function queueRendererRestart(
    renderer: RendererInstance | null,
    field: string,
  ): void {
    if (!renderer) return;
  ...
  async function refreshAll(): Promise<void> {
    ...
    try {
      ...
      const [settings, appNotifications, nativeNotifications] =
        await Promise.all([
  ...
  async function refreshRendererRuntimeStatuses(
    instances: RendererInstance[] = renderers,
  ): Promise<void> {
    if (!hasTauriRuntime()) return;
    ...
    for (const status of statuses) {
      ...
      if (prev) {
        ...
        status.unmotion_zenoh_received_fps = Math.max(
          0,
          (status.unmotion_zenoh_received_frames - prev.unmotionRecv) /
            elapsed,
        );
        status.motion_applied_fps = Math.max(
          0,
          (status.motion_applied_frames - prev.motionApply) / elapsed,
        );
      }
  ...
  async function sendTestNativeNotification(): Promise<void> {
    ...
    try {
      ...
      nativeNotificationStatus = await invoke<NativeNotificationStatus>(
  ...
  async function exportDiagnostics(): Promise<void> {
    ...
    try {
      ...
      diagnosticsExports = await invoke<DiagnosticsExportEntry[]>(
  ...
  async function compressDiagnosticsEntry(
    entry: DiagnosticsExportEntry,
  ): Promise<void> {
    if (!hasTauriRuntime()) {
      message = "Browser preview: diagnostics compression requires Tauri";
  ...
  async function previewDiagnosticsEntry(
    entry: DiagnosticsExportEntry,
  ): Promise<void> {
    if (!hasTauriRuntime()) {
  ...
  async function browseSettingPath(
    field: "avatar_path" | "icon_path",
    kind: "avatar" | "icon",
  ): Promise<void> {
    ...
    try {
      ...
      const setting = await invoke<AvatarSetting>(
  ...
  async function acceptVrmMetadataAndUse(): Promise<void> {
    ...
    try {
      ...
      if (
        useThumbnailForProfileIconOnAccept &&
        modal.metadata.thumbnail_data_url
      ) {
  ...
  async function applySelectedAvatarThumbnail(options: {
    rendererToRestart?: RendererInstance | null;
    ...
    const settingId =
      options.settingId ?? selectedSetting?.manifest_path ?? selectedSetting?.id ?? null;
    const avatarPath = options.avatarPath ?? selectedSetting?.avatar_path ?? null;
    const rendererToRestart =
      options.rendererToRestart ?? restartableRendererForField("icon_path");
  ...
  async function setAppliedMessage(
    applied: number,
    singleMessage: string,
    multiMessage: (applied: number) => string,
  ): Promise<void> {
    if (applied <= 0) return;
  ...
  async function applyRuntimeBackgroundColor(
    ...
    const applied = await applyRendererCommand(
      renderersToApply,
      "set_renderer_clear_color",
      () => rendererClearColorPayload(setting),
    );
    await setAppliedMessage(
      applied,
      "Updated background color",
      (count) => `Updated background color on ${count} renderers`,
    );
  }
  ...
  async function applyRuntimeMotionUpdate(
    ...
    if (
      ...
      await setAppliedMessage(
        applied,
        "Updated motion receivers",
        (count) => `Updated motion receivers on ${count} renderers`,
      );
    }
  }
  ...
  async function applyRuntimePhysicsUpdate(
    ...
    if (
      ...
      await setAppliedMessage(
        applied,
        "Updated motion physics",
        (count) => `Updated motion physics on ${count} renderers`,
      );
      return;
    }
    if (fieldSetIncludes(fields, "debug.show_bone_colliders")) {
      ...
      await setAppliedMessage(
        applied,
        "Updated collider display",
        (count) => `Updated collider display on ${count} renderers`,
      );
    }
  }
  ...
  async function applyRuntimeEnvironmentUpdate(
    ...
    if (fieldSetStartsWith(fields, "environment.color.")) {
      ...
      await setAppliedMessage(
        applied,
        "Updated color adjustment",
        (count) => `Updated color adjustment on ${count} renderers`,
      );
      return;
    }
    if (fieldSetStartsWith(fields, "environment.lighting.")) {
      const applied = await applyRendererCommand(
        ...
        () => rendererLightingPayload(setting),
      );
  ...
  async function applyRuntimeProfileUpdates(
    ...
    const enablingTransparentOnOpaqueRenderer =
      enablingTransparent &&
      rendererToApply !== null &&
      !rendererToApply.transparent;
  ...
  async function resetRendererCamera(
    renderer: RendererInstance | null,
  ): Promise<void> {
    if (!renderer) return;
    if (!hasTauriRuntime()) {
  ...
  function setExpressionOverride(
    renderer: RendererInstance | null,
    name: string,
    weight: number,
  ): void {
  ...
  async function clearExpressionOverrides(
    renderer: RendererInstance | null,
  ): Promise<void> {
    if (!renderer) return;
  ...
  async function matchRendererSpoutToWindow(
    renderer: RendererInstance | null,
    status: RendererRuntimeStatus | null,
  ): Promise<void> {
  ...
  async function setRendererWindow(
    ...
    if (!renderer) return;
    const enablingTransparentOnOpaqueRenderer =
      patch.transparent === true && !renderer.transparent;
    if (!hasTauriRuntime()) {
  ...
  async function setRendererShowAxes(
    renderer: RendererInstance | null,
    enabled: boolean,
  ): Promise<void> {
    if (!renderer) return;
  ...
  async function setRendererShowBoneColliders(
    renderer: RendererInstance | null,
    enabled: boolean,
  ): Promise<void> {
    if (!renderer) return;
  ...
  async function setRendererCameraLock(
    renderer: RendererInstance | null,
    locked: boolean,
  ): Promise<void> {
    if (!renderer) return;
  ...
  async function setRendererCameraOrbitPreset(
    renderer: RendererInstance | null,
  ...
  async function saveRendererCameraToProfile(
    renderer: RendererInstance | null,
  ): Promise<void> {
    if (!renderer) return;
    if (!hasTauriRuntime()) return;
  ...
  async function restoreRendererCameraFromProfile(
    renderer: RendererInstance | null,
  ): Promise<void> {
    if (!renderer) return;
    if (!hasTauriRuntime()) return;
  ...
  async function saveRendererWindowToProfile(
    renderer: RendererInstance | null,
  ): Promise<void> {
    if (!renderer) return;
    if (!hasTauriRuntime()) return;
  ...
  async function restoreRendererWindowFromProfile(
    renderer: RendererInstance | null,
  ): Promise<void> {
    if (!renderer) return;
    if (!hasTauriRuntime()) return;
...
<main class="shell">
  <header class="topbar">
    ...
    <div class="status-strip" aria-label="Renderer status summary">
      ...
      <span class:warn={issueCount > 0}
        ><AlertTriangle size={14} />{issueCount} issues</span
      >
      <span>{message}</span>
      {#if screenshotNoticePath}
        <button
          class="screenshot-notice"
          title={screenshotNoticePath}
          onclick={() => void revealScreenshotFolder()}
        >
      ...
      {#if pendingRendererRestart &&
        !(
          ...
          selectedSetting &&
          sameNormalizedPath(
            pendingRendererRestart.renderer.manifest_path,
            selectedSetting.manifest_path,
          )
        )}
        <span class="restart-notice">
          ...
          <button onclick={() => void restartPendingRenderer()}
            >{$_("renderers.toolbar.restart")}</button
          >
          <button onclick={() => (pendingRendererRestart = null)}
    ...
    <div class="header-actions">
      <ThemeModeSwitch
        ...
        title={appSettings.theme_mode === "system"
          ? `Defaulting to OS ${osTheme}`
          : "Theme override is saved"}
        ariaLabel="Theme"
  ...
  <div class="workspace">
    <aside class="side-rail" aria-label="Primary navigation">
      <button
        class:active={activeTab === "renderers"}
        onclick={() => (activeTab = "renderers")}
      ...
      <div class="rail-footer">
        <button
          class="danger"
          disabled={busy || visibleRenderers.length === 0}
          onclick={stopAll}
      ...
      <section class="view renderers-view">
        ...
        <div class="split">
          <RendererProcessTable
            ...
            iconSrcForManifest={(manifestPath) =>
              iconSrc(settingForManifestPath(manifestPath)?.icon_path ?? null)}
          ...
          <RendererDetailsPanel
            ...
            onSetClearColor={([r, g, b]) => {
              if (!selectedRenderer) return;
              return setRendererClearColor(
                selectedRenderer,
                { r, g, b, a: selectedRenderer.transparent ? 0 : 1 },
                "custom",
              );
            }}
            ...
            onOpenProfile={() => {
              ...
              selectedSettingId =
                launchGroupSettings[0]?.id ?? launchTargetSetting.id;
              activeTab = "settings";
            }}
        ...
        <div class="split settings-split">
          <ProfileSettingList
            ...
            runningCountForManifestPath={(manifestPath) =>
              liveRenderersByManifestPath.get(normalizedPathKey(manifestPath))
                ?.length ?? 0}
            onSelect={(settingId) => {
            ...
            {#if selectedSetting}
              ...
              <div class="setting-editor" onscroll={updateActiveProfileSectionFromScroll}>
                <ProfileIdentitySection
                  ...
                  onUpdateSettingValue={(field, value) =>
                ...
                <ProfileLookSection
                  ...
                  onUpdateSettingValue={(field, value) =>
                    updateSettingValue(field, value)}
                  onApplyLookRecommendation={(look) =>
                    applyLookRecommendation(look)}
                ...
                <ProfileOutputSection
                  ...
                  onUpdateSettingValue={(field, value) =>
                    updateSettingValue(field, value)}
                  onApplySpoutResolutionPreset={(preset) =>
                    applySpoutResolutionPreset(preset)}
