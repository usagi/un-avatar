<script lang="ts">
  ...
  import {
    CAMERA_FOV_FIELDS,
    cameraFovFieldValue,
    cameraWindowBasisValues,
    diagonalFovFromFieldValue,
  } from "./cameraFovOptions";
    ...
    <CameraFovNumberField
      ...
      onChange={(value) =>
        onUpdateSettingValue(
          ...
          diagonalFovFromFieldValue(field.key, value, windowWidth, windowHeight),
        )}
    />
  {/each}
  <small>{$_("profiles.editor.camera_window_basis", {
