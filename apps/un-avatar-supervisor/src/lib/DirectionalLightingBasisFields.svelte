...
<div class="lighting-control-cluster lighting-state-cluster">
	...
	<ProfileToggleField
		...
		checked={enabled}
		disabled={busy}
		onChange={(checked) => onUpdateSettingValue("environment.lighting.directional.enabled", checked)}
	/>
</div>
<div class="lighting-control-cluster lighting-basis-cluster">
	...
	<ProfileToggleField
		...
		onChange={(checked) => onUpdateSettingValue("environment.lighting.directional.follow_camera_yaw", checked)}
	/>
	<ProfileToggleField
		label={$_("profiles.editor.lighting_follow_camera_pitch")}
		checked={followCameraPitch}
		disabled={busy || !enabled}
		onChange={(checked) => onUpdateSettingValue("environment.lighting.directional.follow_camera_pitch", checked)}
	/>
</div>
