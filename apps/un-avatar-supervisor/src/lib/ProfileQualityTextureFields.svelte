<script lang="ts">
  ...
  import {
    BCN_CPU_THREAD_SELECT_OPTIONS,
    TEXTURE_SELECT_FIELDS,
  } from "./qualityOptions";
...
{#each TEXTURE_SELECT_FIELDS as field}
  <ProfileSelectField
...
<ProfileToggleField
  ...
  disabled={busy}
  onChange={(checked) =>
    onUpdateSettingValue("render_quality.processed_texture_cache", checked)}
/>
<ProfileToggleField
  ...
  disabled={busy}
  onChange={(checked) =>
    onUpdateSettingValue("render_quality.skin_tone_matching", checked)}
/>
