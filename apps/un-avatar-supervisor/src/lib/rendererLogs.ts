...
export function filteredLinesForRenderer(
  renderers: readonly RendererLogData[],
  rendererId: number,
  textFilter: string,
): string[] {
...
export function filteredLinesForRendererData(
  renderer: RendererLogData,
  textFilter: string,
): string[] {
  const query = textFilter.trim().toLowerCase();
  if (renderer.stderr_tail.length === 0) return [];
  if (!query) return renderer.stderr_tail;
  return renderer.stderr_tail.filter((line) =>
...
export function rendererLineSeverity(
  line: string,
...
export function defaultRendererLogExpanded(renderer: RendererLogData): boolean {
  return (
    ...
    ((renderer.state === "Exited" || renderer.state === "Crashed") &&
      renderer.stderr_tail.length > 0)
  );
}
