<script lang="ts">
  ...
  import ProfileLookRimFields from "./ProfileLookRimFields.svelte";
...
<details class="effect-panel">
  <summary>
    <span>{$_("profiles.editor.look_rim")}</span>
    <small>{$_("profiles.editor.look_rim_summary")}</small>
    <span class="effect-panel-status"
      >{setting.rim_policy} · {formatFixed(setting.rim_intensity ?? 0.35)}</span
    >
  </summary>
  <ProfileLookRimFields
    {setting}
    {busy}
    {colorDisplayMode}
    {onColorModeChange}
