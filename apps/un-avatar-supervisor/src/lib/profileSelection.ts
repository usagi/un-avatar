...
export function compareAvatarSettings<T extends SortableProfileIdentity>(a: T, b: T): number {
	return (
		a.sort_order - b.sort_order || a.created_at.localeCompare(b.created_at) || a.name.localeCompare(b.name) || a.id.localeCompare(b.id)
	);
}

export function isValidLaunchTarget<T extends ProfileLaunchTargetIdentity>(value: string | null, settings: readonly T[]): boolean {
