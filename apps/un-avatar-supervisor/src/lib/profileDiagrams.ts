...
export function lightingDiagram(setting: LightingDiagramSource): LightingDiagram {
	...
	const cameraOffset: Vec3 = [Math.sin(lon) * Math.cos(lat) * radiusM, Math.sin(lat) * radiusM, -Math.cos(lon) * Math.cos(lat) * radiusM];
	const cameraPos: Vec3 = [target[0] + cameraOffset[0], target[1] + cameraOffset[1], target[2] + cameraOffset[2]];
	const cameraDir = normalize3([cameraPos[0] - target[0], cameraPos[1] - target[1], cameraPos[2] - target[2]]);
	...
	const cameraPitchDeg = setting.lighting_directional_follow_camera_pitch ? radiansToDegrees(Math.asin(clamp(cameraDir[1], -1, 1))) : 0;
	const lightElevation = degreesToRadians(clamp(setting.lighting_directional_elevation_deg + cameraPitchDeg, -89, 89));
	...
	const basisEndM: Vec3 = [rayEndM[0] + baseDir[0] * rayLengthM, rayEndM[1], rayEndM[2] + baseDir[2] * rayLengthM];
