...
<div class="runtime-button-row">
	<button class:active={!renderer.decorations} {disabled} onclick={() => applyWindowPatch(borderlessWindowPatch(renderer))}
	...
	<button
		...
		title={$_("renderers.controls.transparent_title")}>{$_("renderers.controls.transparent")}</button
	>
	<button
		class:active={runtimeStatus?.input_passthrough}
		disabled={disabled || !renderer.transparent}
		onclick={() => applyWindowPatch(clickThroughWindowPatch(runtimeStatus))}>{$_("renderers.controls.click_through")}</button
	>
	<button class:active={renderer.always_on_top} {disabled} onclick={() => applyWindowPatch(topmostWindowPatch(renderer))}
	...
	<button
		...
		title={$_("renderers.controls.minimized_title")}>{$_("renderers.controls.minimized")}</button
	>
</div>
