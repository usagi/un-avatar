...
function appSettingsFromPartial(parsed: Partial<AppSettings>): AppSettings {
  return {
    ...
    ...parsed,
    system_tray_enabled:
      typeof parsed.system_tray_enabled === "boolean"
        ? parsed.system_tray_enabled
        : defaultAppSettings.system_tray_enabled,
    theme_mode:
      parsed.theme_mode === "system" ||
      parsed.theme_mode === "light" ||
      parsed.theme_mode === "dark"
        ? parsed.theme_mode
        : defaultAppSettings.theme_mode,
    quit_behavior:
      parsed.quit_behavior === "ask" ||
      parsed.quit_behavior === "stop_renderers" ||
      parsed.quit_behavior === "leave_renderers"
    ...
    renderer_close_hotkey:
      typeof parsed.renderer_close_hotkey === "string" &&
    ...
    last_selected_setting_id:
      typeof parsed.last_selected_setting_id === "string" &&
      parsed.last_selected_setting_id.trim()
        ? parsed.last_selected_setting_id.trim()
        : null,
    locale:
      typeof parsed.locale === "string" ? parsed.locale : defaultAppSettings.locale,
  };
}
