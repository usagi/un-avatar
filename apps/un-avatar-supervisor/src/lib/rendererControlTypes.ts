...
export type RendererWindowStatus = Pick<
  RendererRuntimeStatus,
  | "input_passthrough"
  | "minimized"
  | "window_position"
...
export type RendererCameraData = Pick<
  RendererInstance,
  "pid" | "window_width" | "window_height"
>;

export type RendererCameraStatus = Pick<
  RendererRuntimeStatus,
  | "camera"
  | "window_inner_size"
>;

export const RENDERER_CLEAR_COLOR_FALLBACK: [number, number, number] = [
  0.12,
  0.14,
  0.18,
];

export function rendererClearColorRgb(
  status: RendererBackgroundStatus | null,
...
export function rendererCameraStatusValues(
  camera: RendererCameraSnapshot,
): { fov: string; radius: string } {
  return {
