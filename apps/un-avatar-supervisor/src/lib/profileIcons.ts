...
export function isThumbnailCachePath(path: string): boolean {
  ...
  return (
    ...
    normalized.includes("/profiles/assets/thumbnails/")
  );
}
...
export function profileIconSrc(
  path: string | null,
  hasRuntime: boolean,
  ...
  if (
    path.endsWith("un-avatar-design-master.png") ||
    path.endsWith("un-avatar-artwork-supervisor.png")
  ) {
