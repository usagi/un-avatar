...
<div class={className} {title} aria-label={ariaLabel || $_("settings.theme.label")}>
  <button
    ...
    onclick={() => onChange("system")}>{$_("theme.system")}</button
  >
  <button
    class:active={mode === "light"}
    onclick={() => onChange("light")}><Sun size={15} />{$_("theme.light")}</button
  >
  <button
    class:active={mode === "dark"}
    onclick={() => onChange("dark")}><Moon size={15} />{$_("theme.dark")}</button
  >
</div>
