...
export function clampedNumberFromInput(event: Event, min: number, max: number, fallback = min): number {
	const value = numberFromInput(event);
	return Math.min(max, Math.max(min, Number.isFinite(value) ? value : fallback));
}

export function clampedNumberFromInputOrFallback(event: Event, min: number, max: number, fallback: number): number {
