...
<div class="toolbar">
	<button class="primary" disabled={busy || (!launchTargetSetting && !launchGroupName)} onclick={() => onLaunch()}
	...
	<RendererLaunchTargetSelect
		...
		{onSelectSetting}
	/>
	<button onclick={() => onRefresh()}><Activity size={16} />{$_("renderers.toolbar.refresh")}</button>
	<label class="toggle-field renderer-toolbar-toggle">
		<input type="checkbox" bind:checked={showStoppedRenderers} />
