...
<div class:active class="expression-row" title={preset}>
  <div class="expression-name">
    ...
    <small
      >{active
        ? $_("renderers.details.expression_overridden")
        : $_("renderers.details.expression_idle")}</small
    >
  </div>
  ...
  <div class="expression-quick-actions">
    <button
      type="button"
      disabled={!hasRunningRenderer || !active}
      onclick={() => setWeight(0)}>0</button
    >
    <button
      type="button"
      disabled={!hasRunningRenderer || weight >= 0.999}
      onclick={() => setWeight(1)}>100</button
    >
  </div>
</div>
