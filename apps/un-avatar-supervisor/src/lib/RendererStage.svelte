...
<section
  class={`renderer-stage renderer-stage-health-${health}`}
  aria-label="Selected renderer summary"
>
  <div class="renderer-stage-identity">
    <div class="stage-avatar">
      <img src={iconUrl} alt="" />
    </div>
    <span class={renderer.state === "Running" ? "storage-badge storage-user renderer-stage-state" : `${rendererStateClass(renderer.state)} renderer-stage-state`}
      >{rendererStateLabel(renderer.state)}</span
    >
  </div>
