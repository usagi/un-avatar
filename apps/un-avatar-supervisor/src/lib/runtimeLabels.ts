...
export type RuntimeStageStatusData = RuntimeQualityStatusLabelData & RendererHealthStatusData;
...
export function startupProgressPercent(status: RuntimeStartupStatusLabelData | null): number {
	const progress = status?.startup_progress;
	if (!progress || progress[1] <= 0) return 0;
	return Math.max(0, Math.min(100, (progress[0] / progress[1]) * 100));
}

export function rendererHealthKind(renderer: RendererHealthRendererData, status: RendererHealthStatusData | null): RendererHealthKind {
	if (renderer.state === "Crashed") return "error";
	if (renderer.state === "Degraded") return "warn";
	if (renderer.state === "Exited") return "idle";
	if (renderer.state === "Stopping" || renderer.state === "Starting") return "pending";
	if (!status?.connected) return "pending";
	if (renderer.spout_enabled) {
		if (!status.spout_available) return "warn";
		if (status.spout_enabled && status.spout_last_send_ok === false) return "warn";
	}
	return "ok";
}
...
export function spoutTimingLabel(status: RuntimeSpoutStatusLabelData | null): string {
	if (!status?.spout_enabled || status.spout_frames_attempted === 0) return "--";
...
export function spoutSenderLabel(status: RuntimeSpoutStatusLabelData | null): string {
	...
	const init = status.spout_sender_initialized ? "initialized" : "not initialized";
	const size =
...
export function runtimeConnectionLabel(status: RuntimeConnectionStatusLabelData | null, labels: RuntimeStatusLabels): string {
