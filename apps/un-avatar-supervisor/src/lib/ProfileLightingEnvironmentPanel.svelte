...
<details class="effect-panel" open>
	<summary>
		...
		<span class="effect-panel-status"
			>{setting.lighting_environment_enabled ? $_("profiles.editor.look_status_on") : $_("profiles.editor.look_status_off")} · {formatFixed(
	...
	<ProfileToggleField
		...
		disabled={busy}
		onChange={(checked) => onUpdateSettingValue("environment.lighting.environment.enabled", checked)}
	/>
	<ColorField
		...
		onChange={(color) => onUpdateSettingValue("environment.lighting.environment.color", color)}
	/>
	<ProfileRangeNumberField
		...
		step={0.01}
		disabled={busy || !setting.lighting_environment_enabled}
		onChange={(value) => onUpdateSettingValue("environment.lighting.environment.intensity", value)}
	/>
</details>
