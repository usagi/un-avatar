<script lang="ts">
  ...
  import {
    ...
    filteredLinesForRendererData,
    rendererLineSeverity,
...
<article class={`logs-card state-${renderer.state}`}>
  <header class="logs-card-header">
    <button
      ...
      onclick={() => onToggleRendererLogExpanded(renderer)}
    >
      {expanded ? "▼" : "▶"}
    ...
    <div class="logs-card-actions">
      <button
        class="ghost-button"
        onclick={() => onCopyRendererLog(renderer)}
        title={$_("logs.renderer.card_copy_title")}
      >
