...
export function expressionOverrideValue(overrides: ExpressionOverrides, rendererId: number | undefined | null, name: string): number {
	if (rendererId == null) return 0;
	return overrides[rendererId]?.[name] ?? 0;
}

export function expressionOverrideCount(overrides: ExpressionOverrides, rendererId: number | undefined | null): number {
	if (rendererId == null) return 0;
...
export function filteredExpressionPresets(presets: string[], filter: string): string[] {
