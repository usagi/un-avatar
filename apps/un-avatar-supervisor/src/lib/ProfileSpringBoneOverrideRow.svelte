<script lang="ts">
  ...
  import {
    SPRING_BONE_MODE_OPTIONS,
    springBoneRecommendedPresets,
...
<div class="spring-bone-override-row">
  ...
  <ProfileSelectField
    ...
    onChange={(value) =>
      onUpdateSettingValue(
        `physics.spring_bone.overrides.${override.category}.mode`,
        value,
      )}
  />
  {#if override.mode !== "authored"}
    <ProfileSpringBoneOverrideFields
      {override}
      {disabled}
      {onUpdateSettingValue}
    />
    <button
      ...
      onclick={() =>
        onUpdateSettingValue(
          `physics.spring_bone.overrides.${override.category}.reset`,
          true,
        )}
    >
  ...
  {#if override.mode === "override_xpbd" && recommendedPresets.length > 0}
    <ProfileSpringBonePresetActions
      {override}
