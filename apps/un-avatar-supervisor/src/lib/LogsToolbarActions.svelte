<script lang="ts">
  ...
  import { Copy, Download, FolderOpen } from "lucide-svelte";
...
<div class="logs-toolbar-actions">
  <button
    class="ghost-button"
    onclick={onCopyAllRendererLogs}
    {disabled}
    title={$_("logs.toolbar.copy_title")}
  >
    <Copy size={14} />
    {rendererLogsCopyFlash ? $_("logs.toolbar.copied") : $_("logs.toolbar.copy_all")}
  </button>
  <button
    class="ghost-button"
    onclick={onSaveAllRendererLogs}
    {disabled}
  ...
  <button
    class="ghost-button"
    onclick={onRevealSupervisorLogsDir}
