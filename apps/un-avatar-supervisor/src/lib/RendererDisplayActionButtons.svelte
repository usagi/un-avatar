...
<div class="runtime-button-row">
	<button
		...
		title={$_("renderers.details.show_axes_title")}>{$_("renderers.details.show_axes")}</button
	>
	<button
		class:active={runtimeStatus?.show_bone_colliders}
		{disabled}
		onclick={() => onSetShowBoneColliders(!(runtimeStatus?.show_bone_colliders ?? false))}
	...
	<button
		...
		title={$_("renderers.details.camera_lock_title")}>{$_("renderers.details.camera_lock")}</button
	>
</div>
