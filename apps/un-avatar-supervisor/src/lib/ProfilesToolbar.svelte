...
<div class="toolbar">
	<button class="primary" data-hint={$_("profiles.hints.toolbar.new")} disabled={busy} onclick={() => onNew()}
		><Plus size={16} />{$_("profiles.actions.new")}</button
	>
	<button
		data-hint={$_("profiles.hints.toolbar.duplicate")}
		disabled={!selectedSettingId || busy}
		onclick={() => onDuplicate(selectedSettingId)}><Copy size={16} />{$_("profiles.actions.duplicate")}</button
	>
	<button
		...
		onkeyup={onCancelDeleteHold}><span class="hold-fill"></span><Trash2 size={16} /><span>{$_("profiles.actions.delete")}</span></button
	>
	<button data-hint={$_("profiles.hints.toolbar.open_folder")} disabled={busy} onclick={() => onOpenFolder()}
