...
export function diagnosticsComparisonSummary(
  left: DiagnosticsExportEntryLabelData,
  right: DiagnosticsExportEntryLabelData,
): string {
  ...
  const timeDelta =
    leftSecs == null || rightSecs == null
      ? "time unknown"
      : `${Math.abs(rightSecs - leftSecs)}s apart`;
...
export function diagnosticsTextureFallbackNote(
  ...
  const fallbackCount =
    numberField(summary, "compression_fallback_count") ?? 0;
  if (fallbackCount <= 0) return null;
  const compressedCount = numberField(summary, "compressed_count") ?? 0;
  if (
    compressedCount === 0 &&
    booleanField(summary, "compression_bc_supported") === false
  ) {
...
export function diagnosticsRendererSpoutLabel(
  snapshot: DiagnosticsRendererSnapshotLabelData,
): string {
...
export function diagnosticsTexturePolicyLabel(
  runtimeStatus: Record<string, unknown>,
  pendingLabel: string,
): string {
  if (booleanField(runtimeStatus, "connected") !== true)
    return pendingLabel;
  const limit = textureModeLabel(
    stringField(runtimeStatus, "texture_resolution_limit"),
  );
  const compression = textureModeLabel(
    stringField(runtimeStatus, "texture_compression"),
  );
  const cache = booleanField(runtimeStatus, "processed_texture_cache");
  const cacheLabel =
    cache == null ? "cache --" : cache ? "cache on" : "cache off";
  return `${limit} / ${compression} / ${cacheLabel}`;
}

export function diagnosticsTextureSummaryLabel(
  runtimeStatus: Record<string, unknown>,
): string {
  const summary = objectField(runtimeStatus, "texture_summary");
...
export function diagnosticsRendererCompareLabel(
  snapshot: DiagnosticsRendererSnapshotLabelData,
): string {
  const runtime = snapshot.connected ? "connected" : "disconnected";
  const spout = snapshot.spoutEnabled
    ? `${snapshot.spoutFailures}/${snapshot.spoutConsecutiveFailures} Spout failures`
    : "Spout off";
...
export function diagnosticsBundleStats(bundle: Record<string, unknown>): DiagnosticsBundleStats {
  ...
  for (const renderer of renderers) {
    ...
    spoutConsecutiveFailures +=
      numberField(runtimeStatus, "spout_consecutive_failures") ?? 0;
    textureImages += numberField(textureSummary, "image_count") ?? 0;
    textureUploadedBytes +=
      numberField(textureSummary, "uploaded_mip_bytes") ?? 0;
    textureCompressed += numberField(textureSummary, "compressed_count") ?? 0;
    textureFallbacks +=
      numberField(textureSummary, "compression_fallback_count") ?? 0;
  }
  return {
    ...
    textureFallbacks,
    nativeNotification:
      stringField(nativeNotifications, "permission_state") ?? "unknown",
    disconnected,
  };
}
...
export function diagnosticsRendererSnapshotMap(
  ...
  for (const renderer of arrayField(bundle, "renderers")) {
    const snapshot = diagnosticsRendererSnapshot(
      objectField(renderer, "info"),
      objectField(renderer, "runtime_status"),
      pendingLabel,
    );
...
export function diagnosticsRendererSnapshot(
  ...
  return {
    ...
    textureNote:
      diagnosticsTextureNote(stringField(runtimeStatus, "note")) ??
      diagnosticsTextureFallbackNote(runtimeStatus),
    textureUploadedBytes:
      numberField(textureSummary, "uploaded_mip_bytes") ?? 0,
    ...
    spoutFramesAttempted:
      numberField(runtimeStatus, "spout_frames_attempted") ?? 0,
    spoutFramesSent: numberField(runtimeStatus, "spout_frames_sent") ?? 0,
    spoutFailures: numberField(runtimeStatus, "spout_frame_failures") ?? 0,
    spoutConsecutiveFailures:
      numberField(runtimeStatus, "spout_consecutive_failures") ?? 0,
    note:
      stringField(runtimeStatus, "note") ??
      stringField(info, "last_stderr") ??
      null,
  };
}

export function diagnosticsRendererLevel(
  snapshot: DiagnosticsRendererSnapshot,
): DiagnosticsLevel {
...
export function diagnosticsRendererCompareLevel(
  ...
  if (after.state === "Crashed" || after.state === "Degraded")
    return "warning";
  ...
  for (const entry of arrayField(bundle, "renderers")) {
    ...
    if (booleanField(runtimeStatus, "spout_enabled") === true && spoutConsecutiveFailures > 0) {
      findings.push({
        ...
        body:
          spoutNote ??
          `${spoutConsecutiveFailures} consecutive failed send attempt${spoutConsecutiveFailures === 1 ? "" : "s"}.`,
      });
...
export function diagnosticsComparisonDetails(
  ...
  return [
    ...
    },
    {
      ...
      level:
        leftStats.renderers === rightStats.renderers &&
        leftStats.connected === rightStats.connected
          ? "info"
          : "warning",
    },
    {
      ...
      level:
        rightStats.issueRenderers > leftStats.issueRenderers ||
        rightStats.notifications > leftStats.notifications
          ? "warning"
          : "info",
    },
    {
      ...
      level:
        leftStats.profiles === rightStats.profiles &&
        leftStats.tray === rightStats.tray
          ? "info"
          : "warning",
    },
    {
      ...
      level:
        leftStats.spoutActive === rightStats.spoutActive
          ? "info"
          : "warning",
    },
    {
      ...
      level:
        rightStats.spoutFailures <= leftStats.spoutFailures &&
        rightStats.spoutConsecutiveFailures === 0
          ? "info"
          : "warning",
    },
    {
      ...
      level:
        ...
        leftStats.textureUploadedBytes ===
          rightStats.textureUploadedBytes &&
    ...
    },
    {
      ...
      level:
        leftStats.nativeNotification === rightStats.nativeNotification
          ? "info"
          : "warning",
    },
    (() => {
...
export function diagnosticsRendererInsights(
  ...
  return arrayField(bundle, "renderers")
    ...
    .sort(
      (a, b) =>
        diagnosticsLevelWeight(b.level) - diagnosticsLevelWeight(a.level) ||
