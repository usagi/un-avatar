<script lang="ts">
	...
	function optionLabel(optionLabelValue: string): string {
		return optionLabelValue.startsWith("profiles.") ? $_(optionLabelValue) : optionLabelValue;
	}
</script>

<label class={className} data-hint={hint}
	>{label}<select {value} {disabled} onchange={(event) => onChange((event.currentTarget as HTMLSelectElement).value)}>
